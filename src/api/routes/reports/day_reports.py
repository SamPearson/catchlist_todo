...
from src.database.reports.models import DayReport
from src.database.reports.service import ReportService
from src.database.reports.repositories import ReportRepository
from src.config.models import db  # Import your SQLAlchemy db instance
...
def get_report(date):
    ...
    report = report_service.get_day_report(user_id, date=report_date)
...
@jwt_required()
def list_reports():
    """List all day reports for user"""
    user_id = get_jwt_identity()
    reports = report_service.list_reports(DayReport, user_id=user_id)
...
def update_report(report_id):
    """Update a day report"""
    user_id = get_jwt_identity()
    report = report_service.get_day_report(user_id, id=report_id)
    if not report:
        return ('', 404)
